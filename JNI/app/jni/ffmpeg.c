                                ...
                                AVSubtitleRect *r)
{
    ...
    if (r->type != SUBTITLE_BITMAP) {
        if (loglevel == 2) 
        	LOGI("sub2video: non-bitmap subtitle\n");
        return;
    }
    if (r->x < 0 || r->x + r->w > w || r->y < 0 || r->y + r->h > h) {
        if (loglevel == 2) 
        	LOGI("sub2video: rectangle (%d %d %d %d) overflowing %d %d\n",
            r->x, r->y, r->w, r->h, w, h
        );
        return;
    }
...
static void sub2video_update(InputStream *ist, AVSubtitle *sub)
{
    ...
    if (sub2video_get_blank_frame(ist) < 0) {
        if (loglevel > 0) 
        	LOGI(               "Impossible to get a blank canvas.\n");
        return;
    }
    dst          = frame->data    [0];
...
static BOOL WINAPI CtrlHandler(DWORD fdwCtrlType)
{
    ...
    default:
        if (loglevel > 0) 
        	LOGI("Received unknown windows signal %ld\n", fdwCtrlType);
        return FALSE;
    }
}
#endif
...
static void ffmpeg_cleanup(int ret)
{
    ...
    if (vstats_file) {
        if (fclose(vstats_file))
            if (loglevel > 0) 
            	LOGI(                   "Error closing vstats file, loss of information possible: %s\n",
...
static void write_packet(OutputFile *of, AVPacket *pkt, OutputStream *ost)
{
    ...
    if (!of->header_written) {
        ...
        if (!av_fifo_space(ost->muxing_queue)) {
            ...
            if (new_size <= av_fifo_size(ost->muxing_queue)) {
                if (loglevel > 0) 
                	LOGI(                       "Too many packets buffered for output stream %d:%d.\n",
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
        ...
        if (ost->frame_rate.num && ost->is_cfr) {
            if (pkt->duration > 0)
                if (loglevel == 2) 
                	LOGI("Overriding packet duration by frame rate, this should not happen\n");
    ...
    if (!(s->oformat->flags & AVFMT_NOTIMESTAMPS)) {
        if (pkt->dts != AV_NOPTS_VALUE &&
            ...
            pkt->dts > pkt->pts) {
            if (loglevel == 2) 
            	LOGI("Invalid DTS: %"PRId64" PTS: %"PRId64" in output stream %d:%d, replacing by guess\n",
                   pkt->dts, pkt->pts,
                   ost->file_index, ost->st->index);
            pkt->pts =
        ...
        if ((st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO || st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) &&
            ...
            if (pkt->dts < max) {
                ...
                if (loglevel == 2) 
                	LOGI("Non-monotonous DTS in output stream "
                ...
                if (loglevel == 2) 
                	LOGI("changing to %"PRId64". This may result "
    ...
    if (ret < 0) {
        if (loglevel > 0) 
        	        LOGI("av_interleaved_write_frame()", ret);
        main_return_code = 1;
...
finish:
    if (ret < 0 && ret != AVERROR_EOF) {
        if (loglevel > 0) 
        	LOGI("Error applying bitstream filters to an output "
                            ...
                            AVSubtitle *sub)
{
    ...
    if (sub->pts == AV_NOPTS_VALUE) {
        if (loglevel > 0) 
        	LOGI("Subtitle packets must have a pts\n");
        if (exit_on_error)
            exit_program(2008);
        return;
    }
    ...
    if (!subtitle_out) {
        ...
        if (!subtitle_out) {
            if (loglevel > 0) 
            	LOGI("Failed to allocate subtitle_out\n");
            exit_program(2009);
        }
    }
                         ...
                         double sync_ipts)
{
    ...
    if (!next_picture) {
        ...
        if (delta0 < 0 &&
            ...
            if (delta0 < -0.6) {
                if (loglevel == 2) 
                	LOGI("Past duration %f too large\n", -delta0);
            } else
                if (loglevel == 2) 
                	LOGI("Clipping frame in rate conversion by %f\n", -delta0);
        ...
        case VSYNC_VSCFR:
            if (ost->frame_number == 0 && delta0 >= 0.5) {
                if (loglevel == 2) 
                	LOGI("Not duplicating %d initial frames\n", (int)lrintf(delta0));
                delta = duration;
                delta0 = 0;
    ...
    if (nb0_frames == 0 && ost->last_dropped) {
        nb_frames_drop++;
        if (loglevel == 2) 
        	LOGI(               "*** dropping frame %d from stream %d at ts %"PRId64"\n",
               ost->frame_number, ost->st->index, ost->last_frame->pts);
    }
    if (nb_frames > (nb0_frames && ost->last_dropped) + (nb_frames > nb0_frames)) {
        if (nb_frames > dts_error_threshold * 30) {
            if (loglevel > 0) 
            	LOGI("%d frame duplication too large, skipping\n", nb_frames - 1);
            nb_frames_drop++;
            return;
        }
  ...
  for (i = 0; i < nb_frames; i++) {
...
#endif
    {
        ...
        if (forced_keyframe) {
            ...
            if (loglevel == 2) 
            	LOGI("Forced keyframe at time %f\n", pts_time);
        }
        ...
        while (1) {
            ...
            if (ret < 0) {
                if (ret != AVERROR(EAGAIN) && ret != AVERROR_EOF) {
                    if (loglevel == 2) 
                    	LOGI(                           "Error in av_buffersink_get_frame_flags(): %s\n", av_err2str(ret));
            ...
            case AVMEDIA_TYPE_AUDIO:
                if (!(enc->codec->capabilities & AV_CODEC_CAP_PARAM_CHANGE) &&
                    ...
                    if (loglevel > 0) 
                    	LOGI(                           "Audio filter graph output is not normalized and encoder does not support parameter changes\n");
                    break;
                }
...
static void print_final_stats(int64_t total_size)
{
    ...
    if(video_size + data_size + audio_size + subtitle_size + extra_size == 0){
        if (loglevel == 2) 
        	LOGI("Output file is empty, nothing was encoded ");
        if (pass1_used) {
            if (loglevel == 2) 
            	LOGI("\n");
        } else {
            if (loglevel == 2) 
            	LOGI("(check -ss / -t / -frames parameters if used)\n");
        }
    }
}
    ...
    if (progress_avio) {
        ...
        if (is_last_report) {
            if ((ret = avio_closep(&progress_avio)) < 0)
                if (loglevel > 0) 
                	LOGI(                       "Error closing progress log, loss of information possible: %s\n", av_err2str(ret));
        }
    }
...
static void do_streamcopy(InputStream *ist, OutputStream *ost, const AVPacket *pkt)
{
    ...
    if (  ost->st->codecpar->codec_id != AV_CODEC_ID_H264
       ...
       && ost->st->codecpar->codec_id != AV_CODEC_ID_VC1
        ...
        if (ret < 0) {
            if (loglevel > 0) 
            	LOGI("av_parser_change failed: %s\n",
                   av_err2str(ret));
            exit_program(2014);
        }
        if (ret) {
...
int guess_input_channel_layout(InputStream *ist)
{
    ...
    if (!dec->channel_layout) {
        ...
        if (loglevel == 2) 
        	LOGI("Guessed Channel Layout for Input Stream "
...
static void check_decode_result(InputStream *ist, int *got_output, int ret)
{
    ...
    if (exit_on_error && *got_output && ist) {
        if (av_frame_get_decode_error_flags(ist->decoded_frame) || (ist->decoded_frame->flags & AV_FRAME_FLAG_CORRUPT)) {
            if (loglevel > 0) 
            	LOGI("%s: corrupt decoded frame in stream %d\n", input_files[ist->file_index]->ctx->filename, ist->st->index);
            exit_program(2018);
        }
    }
}
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (resample_changed) {
        ...
        if (!guess_input_channel_layout(ist)) {
            if (loglevel > 0) 
            	LOGI("Unable to find default channel "
        ...
        for (i = 0; i < nb_filtergraphs; i++)
            if (ist_in_filtergraph(filtergraphs[i], ist)) {
                ...
                if (configure_filtergraph(fg) < 0) {
                    if (loglevel > 0) 
                    	LOGI("Error reinitializing filters!\n");
                    exit_program(2020);
                }
            }
    }
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output, int eof)
{
    ...
    if (ist->st->codecpar->video_delay < ist->dec_ctx->has_b_frames) {
        if (ist->dec_ctx->codec_id == AV_CODEC_ID_H264) {
            ...
            if (loglevel == 2) 
            	LOGI(                   "video_delay is larger in decoder than demuxer %d > %d.\n"
    ...
    if (*got_output && ret >= 0) {
        if (ist->dec_ctx->width  != decoded_frame->width ||
            ...
            if (loglevel == 2) 
            	LOGI("Frame parameters mismatch context %d,%d,%d != %d,%d,%d\n",
                decoded_frame->width,
    ...
    for (i = 0; i < ist->nb_filters; i++) {
        ...
        if (err == AVERROR_EOF) {
            ...
            if (loglevel > 0) 
            	LOGI(                   "Failed to inject frame into filter network: %s\n", av_err2str(err));
            exit_program(2022);
        }
    }
    ...
    if (ist->fix_sub_duration) {
        ...
        if (ist->prev_sub.got_output) {
            ...
            if (end < ist->prev_sub.subtitle.end_display_time) {
                if (loglevel == 2) 
                	LOGI(                       "Subtitle duration reduced from %d to %d%s\n",
...
static int process_input_packet(InputStream *ist, const AVPacket *pkt, int no_eof)
{
    ...
    while (ist->decoding_needed) {
        ...
        if (ret < 0) {
            if (loglevel > 0) 
            	LOGI("Error while decoding stream #%d:%d: %s\n",
    ...
    if (!pkt && ist->decoding_needed && eof_reached && !no_eof) {
        ...
        if (ret < 0) {
            if (loglevel > 0) 
            	LOGI("Error marking filters as finished\n");
            exit_program(2024);
        }
    }
...
static void print_sdp(void)
{
    ...
    if (!sdp_filename) {
        ...
        if (avio_open2(&sdp_pb, sdp_filename, AVIO_FLAG_WRITE, &int_cb, NULL) < 0) {
            if (loglevel > 0) 
            	LOGI("Failed to open sdp file '%s'\n", sdp_filename);
        } else {
    ...
    for (p = pix_fmts; *p != -1; p++) {
        ...
        if (ret < 0) {
            if (ist->hwaccel_id == hwaccel->id) {
                if (loglevel > 0) 
                	LOGI(                       "%s hwaccel requested for input stream #%d:%d, "
        ...
        if (ist->dec_ctx->codec_id == AV_CODEC_ID_DVB_SUBTITLE &&
           (ist->decoding_needed & DECODING_FOR_OST)) {
            ...
            if (ist->decoding_needed & DECODING_FOR_FILTER)
                if (loglevel == 2) 
                	LOGI("Warning using DVB subtitles for filtering and output at the same time is not fully supported, also see -compute_edt [0|1]\n");
        }
...
static int check_init_output_file(OutputFile *of, int file_index)
{
    ...
    if (ret < 0) {
        if (loglevel > 0) 
        	LOGI(               "Could not write header for output file #%d "
...
static int init_output_bsfs(OutputStream *ost)
{
    ...
    for (i = 0; i < ost->nb_bitstream_filters; i++) {
        ...
        if (ret < 0) {
            if (loglevel > 0) 
            	LOGI("Error initializing bitstream filter: %s\n",
...
static int init_output_stream_streamcopy(OutputStream *ost)
{
    ...
    if (ret < 0) {
        if (loglevel > 0) 
        	LOGI(               "Error setting up codec context options.\n");
        return ret;
    }
    ...
    case AVMEDIA_TYPE_AUDIO:
        if (audio_volume != 256) {
            if (loglevel > 0) 
            	LOGI("-acodec copy and -vol are incompatible (frames are not decoded)\n");
            exit_program(2026);
        }
...
static int init_output_stream(OutputStream *ost, char *error, int error_len)
{
    ...
    if (ost->encoding_needed) {
        ...
        if (ost->enc_ctx->bit_rate && ost->enc_ctx->bit_rate < 1000)
            if (loglevel == 2) 
            	LOGI("The bitrate parameter is set too low."
                                         " It takes bits/s as argument, not kbits/s\n");

        ret = avcodec_parameters_from_context(ost->st->codecpar, ost->enc_ctx);
        if (ret < 0) {
            if (loglevel > 0) 
            	LOGI(                   "Error initializing the output stream codec context.\n");
            exit_program(2028);
        }
        /*
                                    ...
                                    AVCodecContext *avctx)
{
    ...
    if (!pts) {
        if (loglevel > 0) 
        	LOGI("Could not allocate forced key frames array.\n");
        exit_program(2029);
    }
...
static void report_new_stream(int input_index, AVPacket *pkt)
{
    ...
    if (pkt->stream_index < file->nb_streams_warn)
        return;
    if (loglevel == 2) 
    	LOGI(           "New %s stream %d:%d at pos:%"PRId64" and DTS:%ss\n",
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (   av_q2d(enc_ctx->time_base) < 0.001 && video_sync_method != VSYNC_PASSTHROUGH
                   && (video_sync_method == VSYNC_CFR || video_sync_method == VSYNC_VSCFR || (video_sync_method == VSYNC_AUTO && !(oc->oformat->flags & AVFMT_VARIABLE_FPS)))){
                    if (loglevel == 2) 
                    	LOGI("Frame rate very high for a muxer not efficiently supporting it.\n"
                ...
                if (!strncmp(ost->enc->name, "libx264", 7) &&
                    ...
                    if (loglevel == 2) 
                    	LOGI(                           "No pixel format specified, %s for H.264 encoding chosen.\n"
                ...
                if (!strncmp(ost->enc->name, "mpeg2video", 10) &&
                    ...
                    ost->filter->filter->inputs[0]->format != AV_PIX_FMT_YUV420P)
                    if (loglevel == 2) 
                    	LOGI(                           "No pixel format specified, %s for MPEG-2 encoding chosen.\n"
                           "Use -pix_fmt yuv420p for compatibility with outdated media players.\n",
                           av_get_pix_fmt_name(ost->filter->filter->inputs[0]->format));
                ...
                if (ost->forced_keyframes) {
                    if (!strncmp(ost->forced_keyframes, "expr:", 5)) {
                        ...
                        if (ret < 0) {
                            if (loglevel > 0) 
                            	LOGI(                                   "Invalid force_key_frames expression '%s'\n", ost->forced_keyframes+5);
                            return ret;
                        }
...
static OutputStream *choose_output(void)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->st->cur_dts == AV_NOPTS_VALUE)
            if (loglevel == 2) 
            	LOGI("cur_dts is invalid (this is harmless if it occurs once at the start per stream)\n");
...
static int check_keyboard_interaction(int64_t cur_time)
{
    ...
    if (key == 'c' || key == 'C'){
        ...
        if (k > 0 &&
            ...
            if (loglevel == 2) 
            	LOGI("Processing command target:%s time:%f command:%s arg:%s",
            ...
            if (loglevel > 0) 
            	LOGI(                   "Parse error, at least 3 arguments were expected, "
...
static void *input_thread(void *arg)
{
    ...
    while (1) {
        ...
        if (flags && ret == AVERROR(EAGAIN)) {
            ...
            ret = av_thread_message_queue_send(f->in_thread_queue, &pkt, flags);
            if (loglevel == 2) 
            	LOGI(                   "Thread message queue blocking; consider raising the "
        ...
        if (ret < 0) {
            if (ret != AVERROR_EOF)
                if (loglevel > 0) 
                	LOGI(                       "Unable to send packet to main thread: %s\n",
                       av_err2str(ret));
            av_packet_unref(&pkt);
            av_thread_message_queue_set_err_recv(f->in_thread_queue, ret);
            break;
...
static int init_input_threads(void)
{
    ...
    for (i = 0; i < nb_input_files; i++) {
        ...
        if ((ret = pthread_create(&f->thread, NULL, input_thread, f))) {
            if (loglevel > 0) 
            	LOGI("pthread_create failed: %s. Try to increase `ulimit -v` or decrease `ulimit -s`.\n", strerror(ret));
            av_thread_message_queue_free(&f->in_thread_queue);
            return AVERROR(ret);
        }
    }
...
static int process_input(int file_index)
{
    ...
    if (exit_on_error && (pkt.flags & AV_PKT_FLAG_CORRUPT)) {
        if (loglevel > 0) 
        	LOGI("%s: corrupt input packet in stream %d\n", is->filename, pkt.stream_index);
        exit_program(2037);
    }
    ...
    if(!ist->wrap_correction_done && is->start_time != AV_NOPTS_VALUE && ist->st->pts_wrap_bits < 64){
        ...
        if (   ist->next_dts == AV_NOPTS_VALUE
            ...
            if (new_start_time > is->start_time) {
                if (loglevel == 2) 
                	LOGI("Correcting start time by %"PRId64"\n", new_start_time - is->start_time);
    ...
    if ((ist->dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO ||
        ...
        if (delta < -1LL*dts_delta_threshold*AV_TIME_BASE ||
            ...
            if (loglevel == 2) 
            	LOGI(                   "Inter stream timestamp discontinuity %"PRId64", new offset= %"PRId64"\n",
    ...
    if ((ist->dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO ||
        ...
        if (is->iformat->flags & AVFMT_TS_DISCONT) {
            if (delta < -1LL*dts_delta_threshold*AV_TIME_BASE ||
                ...
                ifile->ts_offset -= delta;
                if (loglevel == 2) 
                	LOGI(                       "timestamp discontinuity %"PRId64", new offset= %"PRId64"\n",
                       delta, ifile->ts_offset);
                pkt.dts -= av_rescale_q(delta, AV_TIME_BASE_Q, ist->st->time_base);
                if (pkt.pts != AV_NOPTS_VALUE)
                    pkt.pts -= av_rescale_q(delta, AV_TIME_BASE_Q, ist->st->time_base);
            ...
            if ( delta < -1LL*dts_error_threshold*AV_TIME_BASE ||
                ...
                if (loglevel == 2) 
                	LOGI("DTS %"PRId64", next:%"PRId64" st:%d invalid dropping\n", pkt.dts, ist->next_dts, pkt.stream_index);
                pkt.dts = AV_NOPTS_VALUE;
            }
            if (pkt.pts != AV_NOPTS_VALUE){
                ...
                if ( delta < -1LL*dts_error_threshold*AV_TIME_BASE ||
                     delta >  1LL*dts_error_threshold*AV_TIME_BASE) {
                    if (loglevel == 2) 
                    	LOGI("PTS %"PRId64", next:%"PRId64" invalid dropping st:%d\n", pkt.pts, ist->next_dts, pkt.stream_index);
                    pkt.pts = AV_NOPTS_VALUE;
                }
            }
        }
...
static int transcode(void)
{
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        if (!output_files[i]->header_written) {
            if (loglevel > 0) 
            	LOGI(                   "Nothing was written into output file %d (%s), because "
        ...
        if ((ret = av_write_trailer(os)) < 0) {
            if (loglevel > 0) 
            	LOGI("Error writing trailer of %s: %s", os->filename, av_err2str(ret));
            if (exit_on_error)
                exit_program(2039);
        }
    }
            ...
            if (ost) {
                if (ost->logfile) {
                    if (fclose(ost->logfile))
                        if (loglevel > 0) 
                        	LOGI(                               "Error closing logfile, loss of information possible: %s\n",
                               av_err2str(AVERROR(errno)));
                    ost->logfile = NULL;
                }
...
//     }
    ...
    if (loglevel == 2) 
    	LOGI("%"PRIu64" frames successfully decoded, %"PRIu64" decoding errors\n",
